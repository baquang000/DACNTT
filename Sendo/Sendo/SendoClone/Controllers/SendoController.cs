...
namespace SendoClone.Controllers
{
    public class SendoController : Controller
    {
        ...
        public ActionResult Login(String msg,String emailMsg,String passwordMsg,String usernameMsg,String successMsg)
        {
            List<Category> myListCategory = db.getListCategory();
            ...
            if(usernameMsg != "")
            {
                ViewBag.usernameMsg = usernameMsg;
            }
        ...
        public ActionResult Login(User myUser)
        {
            ...
            return RedirectToAction("Login", new { msg  = "Email hoặc mật khẩu không chính xác !" }); 
        }
        ...
        public ActionResult Register(User myUser)
        {
            ...
            {
                usernameMsg += "Tên người dùng không được để trống !";
                error = 1; 
            }else if (user.UserName.Trim().Length < 8)
            {
                usernameMsg += "Tên người dùng phải lớn hơn 8 ký tự !";
                error = 1; 
            }
            ...
            {
                passwordMsg += "Mật khẩu không được để trống !";
                error = 1;
            }
             else if(user.Password.Trim().Length < 6)
            {
                passwordMsg += "Mật khẩu phải lớn hơn 8 ký tự !";
                error = 1; 
            }
            ...
            if(error == 1)
            {
                return RedirectToAction("Login",new {usernameMsg,passwordMsg,emailMsg});
            }
            else
            {
            ...
            db.Add(user);
            db.Save();
            return RedirectToAction("Login",new {successMsg = "Đăng ký tài khoản thành công !"});
            }
